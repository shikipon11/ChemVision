...
export default function ScopeMap({ openReaction, openNote }: { openReaction: (id: string) => void; openNote: (id: string) => void }) {
  ...
  const totalPending = all.filter(item => item.status !== 'ready' && !noteLessons.has(item.id)).length;
  ...
  const matches = (lesson: ScopeLesson) => (!query || [lesson.title, ...lesson.focus, lesson.visual].join(' ').toLowerCase().includes(query))
    && (status === 'all' || (status === 'ready' && lesson.status === 'ready') || (status === 'learnable' && noteLessons.has(lesson.id)) || (status === 'planned' && lesson.status !== 'ready' && !noteLessons.has(lesson.id)));
  ...
  return <section className="scope-map">
    ...
    <div className="scope-lessons">{visible.map(({ lesson, chapter: group }) =>
      <button type="button" key={lesson.id} className={active?.id === lesson.id ? 'scope-lesson is-active' : 'scope-lesson'} onClick={() => setActiveId(lesson.id)} aria-pressed={active?.id === lesson.id}>
        ...
        <span className={lesson.status === 'ready' || noteLessons.has(lesson.id) ? 'scope-state ready' : 'scope-state'}>{lesson.status === 'ready' ? 'アニメーションあり' : noteLessons.has(lesson.id) ? '反応ノートあり' : '追加予定'}</span>
      </button>
    )}</div>
    ...
    {active && <div className="scope-detail" aria-live="polite"><div className="scope-detail-head"><div><span className="eyebrow">LEARNING CONTENT</span><h2>{active.title}</h2></div><button type="button" aria-label="詳細を閉じる" onClick={() => setActiveId(null)}>閉じる ×</button></div>
      ...
      {!active.reactionId && !noteLessons.has(active.id) && <p className="scope-planned-note">このテーマの反応ノート・アニメーションは今後追加予定です。関連する物質は図鑑でも検索できます。</p>}
    </div>}
